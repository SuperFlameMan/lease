...
public class SystemUserController {
    ...
    public Result saveOrUpdate(@RequestBody SystemUser systemUser) {
        ...
        if (systemUser.getPassword()!=null){
            password= DigestUtils.md5Hex(systemUser.getPassword());
    ...
    public Result<Boolean> isUsernameExists(@RequestParam String username) {
        ...
        SystemUser one = service.getOne(systemUserLambdaQueryWrapper);
        Boolean isUseName=false;
        if (one==null){
            isUseName=true;
        }
        return Result.ok(isUseName);
    }
    ...
    public Result updateStatusByUserId(@RequestParam Long id, @RequestParam BaseStatus status) {
        ...
        service.update(systemUserLambdaUpdateWrapper);
        return Result.ok();
    }
}
